...
# Suppress the scheduler warning (false positive when using AMP scaler)
...
def sample_candidates(label, l_max, num_neg):
    ...
    # Generate more candidates than needed to handle collisions with true labels
    num_tries = num_neg + 5  # Extra samples to handle potential collisions
    neg_samples = torch.randint(1, l_max + 1, (N, num_tries), device=label.device)

    # Mask out samples that match the true label
    true_loc_expanded = (label + 1).unsqueeze(1)  # (N, 1)
    valid_mask = (neg_samples != true_loc_expanded)  # (N, num_tries)

    # For each row, take first num_neg valid samples
    for i in range(N):
        valid_indices = valid_mask[i].nonzero(as_tuple=True)[0]
        if len(valid_indices) >= num_neg:
            cand_locs[i, 1:] = neg_samples[i, valid_indices[:num_neg]]
        else:
            # Fallback: fill with available and regenerate rest (rare case)
            cand_locs[i, 1:1 + len(valid_indices)] = neg_samples[i, valid_indices]
            # Fill remaining with simple rejection sampling
            remaining = num_neg - len(valid_indices)
            true_loc = label[i].item() + 1
            for j in range(remaining):
                r = torch.randint(1, l_max + 1, (1,), device=label.device).item()
                while r == true_loc:
                    r = torch.randint(1, l_max + 1, (1,), device=label.device).item()
                cand_locs[i, 1 + len(valid_indices) + j] = r
...
class Trainer:
    ...
    def train(self):
        ...
        # Note: scheduler with gamma=1 has no effect, kept for compatibility
        scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=1000, gamma=1)

        # Mixed precision training (updated API for PyTorch 2.0+)
        use_amp = device == 'cuda' and torch.cuda.is_available()
        scaler = torch.amp.GradScaler('cuda', enabled=use_amp)

        # Track if optimizer has stepped (to avoid scheduler warning)
        ...
        for t in range(self.num_epoch):
                    ...
                    if mask_len <= person_traj_len[0] - 2:  # only training
                        ...
                        if aux:
                            ...
                            if gate1_usage_accum is None:
                                gate1_usage_accum = aux['gate1_usage'].clone()
                            else:
                        ...
                        # Only call scheduler after optimizer has actually stepped
            ...
            if hasattr(self, 'logger'):
                self.logger.log(
                    'epoch:{}, time:{:.2f}, valid_acc:{}'.format(self.start_epoch + t, time.time() - start, acc_valid))
            else:
                print(
                    'epoch:{}, time:{:.2f}, valid_acc:{}'.format(self.start_epoch + t, time.time() - start, acc_valid))
            ...
            if hasattr(self, 'logger'):
                self.logger.log(
                    'epoch:{}, time:{:.2f}, test_acc:{}'.format(self.start_epoch + t, time.time() - start, acc_test))
            else:
