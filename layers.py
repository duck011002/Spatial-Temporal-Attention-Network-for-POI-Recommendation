...
class Embed(nn.Module):
    ...
    def forward(self, traj_loc, poi_coords, vec, traj_len, cand_locs):
        ...
        lon1 = traj_xy[:, :, 0].unsqueeze(2) * math.pi / 180.0 # Note: poi_coords was poi[:, 1:3] -> [lat, lon]. Wait, original load.py haversine uses lon=1, lat=2??
...
class TwoStageGatedMoE(nn.Module):
    def __init__(self, embed_dim, config):
        ...
        # Gate 1 (Mechanism)
        # Input: [hidden, adj_delta_s, adj_delta_t, adj_v, time_emb]
        # time_emb dim: we can reuse embed_dim or a small one. Let's use embed_dim to be safe as time_id is passed as embedding.
        # But wait, input spec says: adj_features (3), time_features (embed_dim).
        gate1_input_dim = embed_dim + 3 + embed_dim 
        self.gate1 = nn.Linear(gate1_input_dim, self.groups)
        
        # Gate 2 (Scenario)
        # Input: [hidden, user_emb, cat_emb (opt), admin_emb (opt)]
        # Assuming user_emb (embed_dim). cat/admin are also embed_dim if present.
    ...
    def forward(self, x, gate1_features, gate2_features, padding_mask):
        # x: (N, L, D)
        # gate1_features: dict with keys 'adj' (N, L, 3), 'time' (N, L, D)
        # gate2_features: dict with keys 'user' (N, L, D), 'cat' (opt), 'admin' (opt)
        # padding_mask: (N, L) - 1 for valid, 0 for pad
        ...
        # --- Gate 1 Forward ---
        # Concat features
        g1_in = torch.cat([x, gate1_features['adj'], gate1_features['time']], dim=-1) # (N, L, D+3+D)
        ...
        g2_in = torch.cat(g2_inputs_list, dim=-1) # (N, L, D_in_2)
        ...
        with torch.no_grad():
            ...
            for idx in flat_g1_idx:
                gate1_usage[idx] += 1

        # Iterate over Groups to compose output
        expert_offset = 0
        for g_idx in range(self.groups):
            num_sub_experts = self.experts_per_group[g_idx]
            
            # 1. Compute Gate 2 Probs for this group
            g2_logits_group = self.gate2_layers[g_idx](g2_in) # (N, L, K_m)
            ...
            # 2. Select Top-1 from Gate 2
            g2_vals, g2_indices = torch.topk(g2_probs_group, self.k2, dim=-1) # (N, L, 1) usually k2=1
            # g2_vals is w2
            
            with torch.no_grad():
                if is_selected_valid.any():
                     flat_g2_idx = g2_indices[is_selected_valid].flatten()
                     for idx in flat_g2_idx:
                         gate2_usage[g_idx][idx] += 1

            # 3. Compute Expert Output
            # w1_group: (N, L) - weight of this group for each token. 0 if not selected.
            ...
            for k in range(num_sub_experts):
                ...
                if mask_k.any():
                    batch_idx = mask_k.nonzero(as_tuple=True) # returns (dim0_idx, dim1_idx)
